...
function PresaleTab() {
  ...
  const hasContract =
    PRESALE_ADDRESS &&
    /^0x[0-9a-fA-F]{40}$/.test(PRESALE_ADDRESS) &&
    !/^0x0{40}$/i.test(PRESALE_ADDRESS);
  ...
  return (
    <div style={{ minWidth: 330 }}>
      <Fieldset legend="Presale Status" width="350px" className="flex flex-col mb-3">
        <Checkbox readOnly checked={isOpen}>
          {isOpen
            ? 'Presale Open'
            : beforeOpen
              ? `Presale Opens in ${timeLeft}`
              : 'Presale Closed'}
        </Checkbox>
      ...
      {showBuy && (
        <div className="flex flex-col items-center justify-center mt-2">
          <Fieldset legend="Receiver" width="350px">
          <Input
            placeholder="Wallet Address"
            value={beneficiary}
            onChange={e => { setBeneficiary(e.target.value); setChecked(false); }}
...
export default function Presale() {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', position: "relative", minWidth: 380 }}>
      <ConnectButton95 />
      <div className="mt-2">
        <Tabs defaultActiveTab="Presale">
          <Tab title="Presale" style={{ cursor: `url(${Cursor.Pointer}), pointer` }}>
            <PresaleTab />
          </Tab>
        </Tabs>
      </div>
      ...
      <div className="flex flex-row justify-center mt-2 mb-0">
        ...
        <a
          href={`https://etherscan.io/address/${PRESALE_ADDRESS}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          <span>[{PRESALE_ADDRESS?.slice(0, 6)}…{PRESALE_ADDRESS?.slice(-4)}]</span>
        </a>
      </div>
    </div>
  );
